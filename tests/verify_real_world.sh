...
size_bytes=$(stat -c %s encrypted.bin)
size_mb=$((size_bytes / 1048576))
if [ "$duration" -gt 0 ]; then
    speed=$((size_mb / duration))
    echo "    Encryption took ${duration}s (${speed} MB/s)"
else
    echo "    Encryption took <1s"
fi
...
unzip -o decrypted.zip || true

# 6. Verify Integrity
echo "[*] Checking MD5..."
md5sum -c original.md5 || true

echo "=== DEBUG COMPARISON ==="
echo "Comparing input.zip and decrypted.zip..."
cmp -l input.zip decrypted.zip | head -n 20
