...
AES128Batcher::AES128Batcher(VulkanContext *ctx) : ctx(ctx) {
  DEBUG_PRINT("Initializing AES-128 Batcher...");
  ...
  DEBUG_PRINT("AES-128 Batcher Initialized Successfully");
}

AES128Batcher::~AES128Batcher() {
...
bool AES128Batcher::submit(const unsigned char *in, unsigned char *out,
  ...
  // 2. Setup params - AES-128 EXTENDED Layout (same as AES-256)
  ...
  ubo[0] = (len + 15) / 16; // batchSize (number of 16-byte blocks)
  ubo[1] = 10;              // numRounds for AES-128

  // DEBUG: Print input key and IV
  DEBUG_PRINT("Key: %02x %02x %02x %02x %02x %02x %02x %02x...", key[0], key[1],
              key[2], key[3], key[4], key[5], key[6], key[7]);
  DEBUG_PRINT("IV:  %02x %02x %02x %02x %02x %02x %02x %02x...", iv[0], iv[1],
  ...
  memcpy(ubo + 4, w, 176);  // RoundKey at offset 16 bytes (only 44 words used)
...
void AES128Batcher::createDescriptors() {
...
void AES128Batcher::createPipeline() {
  DEBUG_PRINT("Loading AES-128 shader...");
  auto code = readFile("/usr/local/lib/aes128_ctr.spv");
...
void AES128Batcher::createCommandBuffer() {
...
void AES128Batcher::createSyncObjects() {
